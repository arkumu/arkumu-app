"""Prune all derived triples for selected organizations.

This removes any triples that were created by ``derive_project_relationships``
or other post-processing that marks triples with ``is_derived=True``.

Usage (compose dev):

    docker compose -f docker-compose.dev.yml run --rm django \\
      python manage.py prune_derived_triples --organization khm
...
class Command(BaseCommand):
    help = "Delete all derived triples (is_derived=True) for the given organization(s)."
    ...
    def handle(self, *args, **options):
        ...
        for org_code in sorted(org_codes):
            ...
            if dry_run:
                self.stdout.write(
                    self.style.HTTP_INFO(
                        f"[DRY RUN] Organization={org_code}: derived triples={total}"
                    )
                )
                continue
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Organization={org_code}: deleted {deleted} derived triples"
                )
            )
        ...
        if not dry_run:
            self.stdout.write(
                self.style.MIGRATE_HEADING(
                    f"Prune complete. Total deleted derived triples: {total_deleted}"
                )
            )
