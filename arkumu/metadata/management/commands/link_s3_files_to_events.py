"""Link S3 file objects to their owning event resources in bulk."""
...
class Command(BaseCommand):
    help = (
        "Link unassigned S3FileObject rows to their event resources using the "
        "filename-to-resource matching heuristics, ensuring files attach to events "
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Event linking complete. Processed: {processed}, Linked: {linked}, Ambiguous: {ambiguous}, Errors: {errors}"
            )
        )
