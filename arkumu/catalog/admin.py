...
from arkumu.catalog.models import (
    PreviewImages,
    ProjectDetailIndex,
    ProjectIndex,
    ProjectRecordIndex,
...
class ProjectIndexAdmin(admin.ModelAdmin):
    ...
    search_fields = ("uri", "title", "subtitle", "institution_label", "categories", "actor_names")
    list_filter = ("org_code", "public_access_level", "is_public_approved", "is_derived")
    readonly_fields = (
        "project_resource",
        "uri_link",
        "source_updated_at",
    ...
    fieldsets = (
        ...
        ("Content", {
            "fields": ("title", "subtitle", "image", "institution_label", "year_range")
        }),
        ("Arrays", {
            "fields": ("categories_display", "actors_display", "digital_objects_display"),
            "classes": ("collapse",),
        }),
        ("Visibility", {
            "fields": ("public_access_level", "is_public_approved", "is_derived")
        }),
        ("Metadata", {
    ...
    def uri_link(self, obj):
        if obj.uri:
            return format_html('<a href="{}" target="_blank">{}</a>', obj.uri, truncatechars(obj.uri, 60))
        return "-"

    @admin.display(description="Categories")
    def category_count(self, obj):
        count = len(obj.categories) if obj.categories else 0
        return count

    @admin.display(description="Actors")
    def actor_count(self, obj):
        count = len(obj.actor_names) if obj.actor_names else 0
        return count
    ...
    def categories_display(self, obj):
        if not obj.categories:
            return "-"
        return format_html("<br>".join(obj.categories[:20]))
    ...
    def digital_objects_display(self, obj):
        if not obj.digital_object_paths:
            return "-"
        return format_html("<br>".join(truncatechars(p, 80) for p in obj.digital_object_paths[:10]))
...
class ProjectRecordIndexAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("Arrays", {
            "fields": ("categories_display", "actors_display", "years_display"),
            "classes": ("collapse",),
        }),
        ("Visibility", {
            "fields": (
...
@admin.register(ProjectDetailIndex)
class ProjectDetailIndexAdmin(admin.ModelAdmin):
    list_display = (
        "title_short",
        "org_code",
        "institution_label",
    ...
    list_display_links = ("title_short",)
    search_fields = ("uri", "title", "subtitle", "description", "institution_label")
    list_filter = (
        "org_code",
        "public_access_level",
        "is_public_approved",
    ...
    fieldsets = (
        ...
        ("Categories & Actors", {
            "fields": ("categories_display", "actors_display"),
        }),
        ("Events", {
        ...
        ("Digital Objects", {
            "fields": ("digital_objects_display",),
            "classes": ("collapse",),
        }),
    ...
    @admin.display(description="Act.")
    def actor_count(self, obj):
        return len(obj.actors) if obj.actors else 0
